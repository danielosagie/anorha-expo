...
void main() {
    ...
    vec2 noiseUV = aspectUV - movementDirection * animTime * 0.1;

    float noise = snoise(vec3(noiseUV * 3.0, animTime));
    ...
    vec3 baseGradient = mix(vec3(0.4, 0.4, 0.4), vec3(0.8, 0.8, 0.8), edgeDistance * 1.5 + noise * 0.3);

    vec3 auroraColor = baseGradient * colorTint;
